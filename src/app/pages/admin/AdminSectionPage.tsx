...
const defaultNote =
  "This section is scaffolded for future content and data management. Final route wiring, live data, permissions, publishing workflow, and Firebase integration will be completed later in GitHub/Firebase Studio.";
...
function Badge({ label, className }: { label: string; className: string }) {
  return (
    <span className={`rounded-md px-2 py-1 text-[10px] font-semibold ${className}`}>
      {label}
    </span>
  );
}
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4">
      ...
      <p className="text-xs text-slate-500">
        {link?.note ?? "Route relationship and publishing mapping."}
      </p>
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-slate-200 bg-white p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              className="inline-flex items-center gap-1 rounded-lg bg-blue-700 px-3 py-2 text-xs font-semibold text-white"
            >
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
        <div className="border-b border-slate-200 p-4 text-sm font-semibold text-slate-800">
          Management Table
        </div>

        {tableRows.length === 0 ? (
          <div className="m-4 rounded-xl border border-dashed border-slate-300 p-5 text-sm text-slate-500">
            No items yet
          </div>
        ) : (
              ...
              <tbody>
                {tableRows.map((row) => (
                  <tr key={`${row.name}-${row.lastUpdated}`} className="border-t border-slate-100">
                    ...
                    <td>
                      <Badge label={row.status} className={statusClasses[row.status]} />
                    </td>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 bg-white p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {(Object.keys(statusClasses) as AdminSectionStatus[]).map((key) => (
              <Badge key={key} label={key} className={statusClasses[key]} />
            ))}
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {relatedSections.length ? (
              ...
              <span className="text-xs text-slate-500">No linked sections yet.</span>
            )}
          </div>
        </div>
      </div>
